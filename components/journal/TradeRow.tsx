...
export function TradeRow({ trade, onDelete, onSave }: {
  ...
  // Controlled state for every manual field
  const [setupName, setSetupName]         = useState(trade.setup_name     ?? '');
  const [riskDollars, setRiskDollars]     = useState(trade.risk_dollars   ?? '');
  const [slTicks, setSlTicks]             = useState(trade.sl_ticks       ?? '');
  const [tp1, setTp1]                     = useState(trade.tp1            ?? '');
  const [tp2, setTp2]                     = useState(trade.tp2            ?? '');
  const [tp3, setTp3]                     = useState(trade.tp3            ?? '');
  const [dexReached, setDexReached]       = useState(trade.dex_reached    ?? false);
  const [mfeTicks, setMfeTicks]           = useState(trade.mfe_ticks      ?? '');
  const [maeTicks, setMaeTicks]           = useState(trade.mae_ticks      ?? '');
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      ...
      >
        ...
        {open ? <ChevronUp size={16} className="ml-auto shrink-0 text-muted-foreground" />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                type="number" min="0"
                value={mfeTicks}
                onChange={e => setMfeTicks(e.target.value)}
                placeholder="e.g. 20"
                className="h-8 rounded-md border border-input bg-background px-2 text-sm focus:outline-none focus:ring-1 focus:ring-ring"
              />
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                type="number" min="0"
                value={maeTicks}
                onChange={e => setMaeTicks(e.target.value)}
                placeholder="e.g. 4"
                className="h-8 rounded-md border border-input bg-background px-2 text-sm focus:outline-none focus:ring-1 focus:ring-ring"
              />
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                value={dexReached ? 'yes' : 'no'}
                onChange={e => setDexReached(e.target.value === 'yes')}
                className="h-8 rounded-md border border-input bg-background px-2 text-sm focus:outline-none focus:ring-1 focus:ring-ring"
              >
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                value={ruleAdherence ? 'yes' : 'no'}
                onChange={e => setRuleAdherence(e.target.value === 'yes')}
                className="h-8 rounded-md border border-input bg-background px-2 text-sm focus:outline-none focus:ring-1 focus:ring-ring"
              >
                <option value="no">No</option>
                <option value="yes">Yes</option>
